    ...
    clickableResources =
      model.clickableResources
        |> List.map
            (\res ->
              let
                count = model.counts |> Dict.get res.name |> Maybe.withDefault 0
              in
              viewResource count True Noop res
            )
...
viewResource count colouredBackground clickMsg resource =
  H.button
    ...
    , A.style "padding" "8px"
    , A.style "background" (if colouredBackground then resource.backgroundColor else "white")
    ]
    [ H.p [ margin0 ]
